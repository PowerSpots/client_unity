...
namespace RTS
{
    public abstract class WorldBase
    {
        ...
        void DispatchOptimizeCreate(EntityBase entity)
        {
            ...
            try
            {
                OnEntityOptimizeCreated?.Invoke(entity);
            }
            catch (Exception e)
            {
        ...
        // 定义角色Entity的组件filter
        string[] player_com_filter = new string[] 
        {
        ...
        public List<EntityBase> GetPlayerList()
        {
            if (!isGetHashCode)
            {
                ...
                filterNameHashCode = group.StringArrayToInt(player_com_filter);
            }
            return group.GetEntityByFilter(filterNameHashCode, player_com_filter);
        }
        #endregion
#endif
        #endregion

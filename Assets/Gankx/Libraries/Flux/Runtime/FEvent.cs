...
//			return (range.start > start && range.start < end) || (range.end > start && range.end < end );
	...
	public class FEvent : FObject
	{
        ...
        [HideInInspector]
        public bool LockFrameRange = false;
        ...
        /// @brief Used to setup your own code for when the sequence is preloaded
...
//				Trigger( timeSinceTrigger );
//
////			OnUpdateEventEditor( timeSinceTrigger );
//
//			if( framesSinceTrigger == Length )
//				Finish();
//
...
//		}
		 ...
		 * @param timeSinceTrigger How much time passed since TriggerFrame
		 */ 
//        protected virtual void OnUpdateEventEditor( float timeSinceTrigger )
//		{
//			OnUpdateEvent( timeSinceTrigger );
//		}
        ...
        protected void Error(string title, string msg)
        {
            ...
            Debug.LogError(string.Format("{0}.{1}  on Sequence[{2}] at Owner[{3}] occurred error:{4}",
                    typeName, title, Sequence.gameObject.name, Owner.gameObject.name, msg));
        }
        ...
        {
            string typeName = GetType().Name;
            Debug.LogWarning(string.Format("{0}.{1} on Sequence[{2}] at Owner[{3}] occurred warning：{4}",
                typeName, title, Sequence.gameObject.name, Owner.gameObject.name, msg));
        }
	}

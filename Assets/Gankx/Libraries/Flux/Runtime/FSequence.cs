...
namespace Flux
{
    ...
    public class FSequence : FObject
	{
        ...
        public const float MIN_TICK_FRAMETIME_THRESHOLD = 0.05f;  // 20fps

        private static bool sInterleaveTick = false;
        private static int SeqPlayedCount = 0;
        private int SeqPlayingId = 0;
...
//		public FTimeline GlobalTimeline { get { return _globalTimeline; } }
        ...
        public bool IsCaptured { get { return _isCaptured; } set { _isCaptured = value; } }
	    ...
	    public void SetActiveContainers(List<string> containerNames)
	    {
            for (int i = 0; i < _containers.Count; i++)
            {
                bool isEnable = containerNames.Contains(_containers[i].name);                
                _containers[i].enabled = isEnable;                
            }
        }
		...
		public void SetCurrentTimeEditor( float time )
		{
		    ...
		    { // 在运行时修改Event字段属性会触发到此函数调用，会将Speed改为-1，这不是预期结果，所以一下代码仅在非运行时生效
		...
		public float CurrentTime {
			...
			private set {
				_currentTime = value;
				CurrentFrame = _currentTime < 0 ? -1 : (IsPlayingForward ? Mathf.FloorToInt(_currentTime * _frameRate) : Mathf.CeilToInt(_currentTime*_frameRate));/*Mathf.RoundToInt(_currentTime * _frameRate);*/
			}
		}

		public bool FrameChanged { get { return _lastFrame != _currentFrame; } }

        /// @brief Set the sequence's owner.
        /// This is called before call the Play
        public void SetOwner(Transform owner, string ownerName = "", bool selectSet = false)
        {
            foreach (FContainer container in _containers)
            {
                container.SetOwner(owner, ownerName, selectSet);
            }
        }

	    public void SetOwnerOfFirstTimeline(Transform owner)
	    {
	        foreach (FContainer container in _containers)
	        {
	            ...
	            container.Timelines[0].SetOwner(owner);
	        }
	    }
	    ...
	    public void SetOwnerByTimelineTag(Transform owner, string tag)
	    {
	        ...
	        foreach (FContainer container in _containers)
	        {
	            ...
	            container.SetOwnerByTimelineTag(owner, tag);
	        }
        }
	    ...
	    public Transform GetOwnerByTimelineTag(string tag)
	    {
	        ...
	        foreach (FContainer container in _containers)
	        {
	            ...
	            Transform result = container.GetOwnerByTimelineTag(tag);
        ...
        public Transform GetOwnerOfFirstTimeline()
	    {
	        ...
	        if (Containers[0].Timelines.Count <= 0)
	        {
                return null;
	        }

	        return Containers[0].Timelines[0].Owner;
	    }

        /// @brief Does the sequence belong to then owner.
        public bool BelongTo(Transform owner)
        {
            foreach (FContainer container in _containers)
            {
		...
		public void CreateTrackCaches()
		{
			for( int i = 0; i != _containers.Count; ++i )
			{
				...
				for( int j = 0; j != timelines.Count; ++j )
				{
					...
					{
//						if( (track.RequiresRuntimeCache() || track.AlwaysUseCache || (track.RequiresEditorCache() && !_isPlayingForward) ) && !track.HasCache )
		...
		public void Play( float startTime )
		{
		    ...
		    SendMessage("OnSequenceStart", SendMessageOptions.DontRequireReceiver);
		    ...
		    Profiler.BeginSample("FSequence Update");
		...
		public void Stop( bool reset )
		{
		    ...
		    SendMessage("OnSequenceStop", SendMessageOptions.DontRequireReceiver);
#if FLUX_DEBUG
			Debug.Log ("Stop");
#endif
			_isPlaying = false;
			_isPlayingForward = Speed*Time.timeScale >= 0;//true;
...
//			}
            ...
            bool bNeedTick = sInterleaveTick ? (SeqPlayingId % 2 == Time.frameCount % 2) : true;
            float delta = time - _lastUpdateTime;

            if (bNeedTick || delta > MIN_TICK_FRAMETIME_THRESHOLD/*20 fps*/)
			{
                //float delta = time - _lastUpdateTime;
	    ...
	    private void LoopOrPause(bool loop)
	    {
            if (loop)
            {
                Stop();

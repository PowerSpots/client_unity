...
{
	[System.Serializable]
	public class FSequenceEditor : FEditorList<FContainerEditor>//ScriptableObject
	{
...
//			_commentTrackRect = rect;
//			_commentTrackRect.xMin += VERTICAL_SCROLLER_WIDTH;
//			_commentTrackRect.xMax -= RIGHT_BORDER;
//			_commentTrackRect.height = FTimelineEditor.TRACK_HEIGHT;

//			rect.yMin += FTimelineEditor.TRACK_HEIGHT;
...
//			_timeScrubberRect.yMin = _commentTrackRect.yMin;
			...
			if( !Application.isPlaying && sequenceChanged )
			{
				foreach( FContainerEditor containerEditor in Editors )
				{
					foreach( FTimelineEditor timelineEditor in containerEditor.Editors )
					{
						...
						{
							if( trackEditor.Track.CanTogglePreview && !trackEditor.Track.PreviewDirtiesScene && !trackEditor.Track.HasCache )
							{
								trackEditor.OnTogglePreview( true );
							}
						}
					}
				}
...
//			objs.Add( _editorCache );
		...
		public void OnGUI()
		{
			...
			if( Event.current.type == EventType.KeyDown && EditorGUIUtility.keyboardControl == 0 )
			{
			    switch (Event.current.keyCode)
			    {
                    ...
                    case KeyCode.LeftArrow:
			            {
                            // Move one frame of all selected events to the left 
                            MoveEvents(-1);
                            Event.current.Use();
                            break;
                        }
                    case KeyCode.RightArrow:
                        {
                            // Move one frame of all selected events to the right
                            MoveEvents(1);
                            Event.current.Use();
                            break;
                        }
...
//			Handles.color = FGUI.GetLineColor();
//			Handles.DrawLine( Rect.min + new Vector2(_viewRect.xMin, Height), _viewRect.min + new Vector2(_sequenceRect.width, Height) );
            ...
            if (newT != Sequence.CurrentFrame && !Application.isPlaying)
			{
				if( _renderingOnEditorWindow )
...
//				Handles.color = Color.black;
		...
		public void TurnOnAllPreviews( bool on )
		{
			foreach( FContainerEditor containerEditor in Editors )
			{
				foreach( FTimelineEditor timelineEditor in containerEditor.Editors )
				{
					foreach( FTrackEditor trackEditor in timelineEditor.Editors )
					{
						if( trackEditor.Track.CanTogglePreview )
						{
							if( on )
							{
								if( !trackEditor.Track.HasCache )
									trackEditor.OnTogglePreview( true );
							}
							else
							{
								if( trackEditor.Track.HasCache )
									trackEditor.OnTogglePreview( false );
							}
						}
					}
				}
...
//			Sequence.SetCurrentTimeEditor( time );
//			int frame = Sequence.GetCurrentFrame();
        ...
        {
            // TODO jackey
            //GameMovieCaptureFromScene capture = FindObjectOfType<GameMovieCaptureFromScene>();
            //if (capture)
            //{
            //    capture.StartSequenceCapture(Sequence);
            //}
        }

...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    private void SafeCallLoad(string strFileName) {
        strFileName = strFileName.Trim();

#if UNITY_EDITOR || UNITY_STANDALONE
        string mediaPath = string.Format("{0}/../data/localimage/{1}", Application.dataPath, strFileName);
#else
		string mediaPath = string.Format("{0}/data/localimage/{1}", Application.persistentDataPath, strFileName);
#endif
        if (File.Exists(mediaPath)) {
            Call_Load("file://" + mediaPath, 0);
        }
        else {
            Call_Load(string.Format("data/localimage/{0}", strFileName), 0);
        }
    }
	...
	{
		strURL = strURL.Trim();
	    strURL = string.Format("data/localimage/{0}", strURL);

...
namespace XLua.LuaDLL
{
	...
	public partial class Lua
	{
       ...
       const string LUADLL = "xlua";
	    ...
	    public static void lua_pushlstring(IntPtr L, byte[] str, int length) {
	        if (str == null) {
	            lua_pushnil(L);
	        } else {
	            xlua_pushlstring(L, str, length);
	        }
	    }
	    ...
	    public static void lua_pushdataboat(IntPtr luaState, DataBoat buf) {
	        if (null == buf.data || buf.data.Length <= 0 || buf.len > buf.data.Length) {
	            return;
	        }

	        lua_pushlstring(luaState, buf.data, buf.len);
	    }
		...
		[DllImport(LUADLL, CallingConvention = CallingConvention.Cdecl)]
		public static extern int luaopen_struct(IntPtr L);//[,,m]
    }
}

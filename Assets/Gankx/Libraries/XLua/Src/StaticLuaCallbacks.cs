...
namespace XLua
{
    ...
    public partial class StaticLuaCallbacks
    {
        ...
        public static int loader(IntPtr L)
        {
            ...
            // Load bytes
            byte[] buf;
            int bufSize = LuaLoader.Load(fileName, out buf);
            if (bufSize <= 0)
            {
                LuaAPI.lua_pushnil(L);
                LuaAPI.lua_pushstring(L, string.Format("cannot open {0}: no such file or file is empty!", fileName));

                return 2;
            }

            if (LuaAPI.xluaL_loadbuffer(L, buf, bufSize, fileName) != 0)
            {
                LuaAPI.lua_pushnil(L);
        ...
        [MonoPInvokeCallback(typeof(LuaCSFunction))]
		internal static int LoadStruct(RealStatePtr L)
		{
			return LuaAPI.luaopen_struct(L);
		}
        ...
        [MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
        public static int new_sized_table(IntPtr L)
        {
            int asize = LuaAPI.xlua_tointeger(L, 1);
            int hsize = LuaAPI.xlua_tointeger(L, 2);
            LuaAPI.lua_createtable(L, asize, hsize);
            return 1;
        }    

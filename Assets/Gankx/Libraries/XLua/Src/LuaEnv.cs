    ...
    public class LuaEnv : IDisposable
    {
        ...
        public LuaEnv()
        {
            ...
            lock(luaEnvLock)
            {
				...
				AddBuildin("struct", StaticLuaCallbacks.LoadStruct);

                LuaAPI.lua_pushstdcallcfunction(L, new LuaCSFunction(StaticLuaCallbacks.loadfile));
                LuaAPI.xlua_setglobal(L, "loadfile");

                LuaAPI.lua_pushstdcallcfunction(L, new LuaCSFunction(StaticLuaCallbacks.new_sized_table));
                LuaAPI.xlua_setglobal(L, "new_sized_table");                
        ...
        public bool IsTrue(string chunk, string chunkName = "chunk", LuaTable env = null)
        {
            var objects = DoString("return " + chunk, chunkName, env);
            var ret = objects[0] as bool? ?? false;
            return ret;
        }

...
#if UNITY_EDITOR
//            else if(!XluaGenConfig.IsTypeGenerated(type) && !XluaGenConfig.CanUseReflection && Application.isPlaying)
//            {
//                UnityEngine.Debug.LogError(string.Format("{0} not gen, using reflection instead-->运行时使用反射，建议在XluaConfig中进行配置", type));
////                return false;
//            }
#endif
            else
        ...
        public void PushAny(RealStatePtr L, object o)
        {
            ...
            else if (o is DataBoat) {
                DataBoat buf = (DataBoat)o;
                LuaAPI.lua_pushdataboat(L, buf);
            } 
        ...
        bool tryGetPushFuncByType<T>(Type type, out T func) where T : class
        {
            if (push_func_with_type == null)
            {
                push_func_with_type = new Dictionary<Type, Delegate>()
                {
                    ...
                    {typeof(DataBoat), new Action<RealStatePtr, DataBoat>(LuaAPI.lua_pushdataboat) },

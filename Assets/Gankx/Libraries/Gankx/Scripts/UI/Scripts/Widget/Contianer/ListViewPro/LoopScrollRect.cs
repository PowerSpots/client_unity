...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [HideInInspector]
        [NonSerialized]
        public LoopScrollSendLuaSource dataSource = LoopScrollSendLuaSource.Instance;
        ...
        //==========LoopScrollRect==========
        private void ReturnObjectAndSendMessage(Transform item)
        {
            dataSource.ReturnItemToPool(transform , item);
            prefabSource.DelObject(item.gameObject);
        }
...
//                objectsToFill = null;
        ...
        public void RefreshCellByIndex(int dataIndex)
        {
             //todo 
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        dataSource.SetItemData(transform, content.GetChild(i), content.GetChild(i).GetSiblingIndex(), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefreshCellsFromEndFraming()
        {
            if (!Application.isPlaying || !this.isActiveAndEnabled)
            {
                return;
            ...
            StopCoroutine("RefreshCellsFromEndFramingCoroutine");
            StartCoroutine(RefreshCellsFromEndFramingCoroutine());
        }

        public IEnumerator RefreshCellsFromEndFramingCoroutine()
        {
            ...
            for (int i = content.childCount - 1; i >= 0; i--)
            {
                ...
                if (itemTypeStart >= 0)
                {
                    itemTypeStart--;
                    {
                        dataSource.SetItemData(transform, childTransform, childTransform.GetSiblingIndex(), itemTypeStart);
                    }
                }
                else
                {
        ...
        public void RefreshCellsFromEnd()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                itemTypeEnd = totalCount;
                ...
                for (int i = content.childCount - 1; i >= 0 ; i--)
                {
                    ...
                    {
                        itemTypeStart--;
                        dataSource.SetItemData(transform, content.GetChild(i), content.GetChild(i).GetSiblingIndex(), itemTypeStart);
                    }
                    else
                    {
                        ReturnObjectAndSendMessage(content.GetChild(i));
            ...
            SetNormalizedPosition(1, 1);
            // 会被认为是向下拉列表，产生向上的滚动
            StartCoroutine("_StopMoveFORfreshEnd");
        }

        //从最后位置向前刷新
        public void RefillCellsFromEnd(int offset = 0)
        {
            Profiling.Profiler.BeginSample("RefillCellsFromEnd");
            ...
            Profiling.Profiler.BeginSample("ReturnObjectAndSendMessage");
            ...
            Profiling.Profiler.BeginSample("NewItemAtEnd");
            ...
            while (sizeToFill > sizeFilled)
            {
                ...
                if (size <= 0)
                {
//                    Debug.LogError("RefillCells Error: size == 0");
                    break;
                }
                sizeFilled += size;
            }
        ...
        private IEnumerator _StopMoveFORfreshEnd()
        {
            yield return null;
            StopMovement();
            SetNormalizedPosition(1, 1);
...
//                    Debug.LogError("RefillCells Error: size == 0");
        ...
        {
            Profiling.Profiler.BeginSample("InstantiateNextItem");

            Profiling.Profiler.BeginSample("1 InstantiateNextItem GetObject");
            RectTransform nextItem = prefabSource.GetObject(content).GetComponent<RectTransform>();
            Profiling.Profiler.EndSample();

            Profiling.Profiler.BeginSample("2 InstantiateNextItem SetActive");
            nextItem.gameObject.SetActive(true);
            Profiling.Profiler.EndSample();

            Profiling.Profiler.BeginSample("3 InstantiateNextItem SetItemData");
            nextItem.localScale = Vector3.one;
            dataSource.SetItemData(transform, nextItem, nextItem.GetSiblingIndex(), itemIdx);
            Profiling.Profiler.EndSample();
        ...
        protected void UISoftNewObjHandler()
        {
        ...
        public virtual void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (LuaService.ContainsInstance())
            {
                ...
                if (window != null)
                {
                    LuaService.instance.FireEvent("OnPanelMessage", window.panelId, window.id , "OnListViewProStartDrag");
                }
            }
        }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (LuaService.ContainsInstance())
            {
                ...
                if (window != null)
                {
                    LuaService.instance.FireEvent("OnPanelMessage", window.panelId, window.id , "OnListViewProStopDrag");
                }
            }
        }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            if (LuaService.ContainsInstance())
            {
                ...
                if (window != null)
                {
                    LuaService.instance.FireEvent("OnPanelMessage", window.panelId, window.id, "OnListViewProDrag" , pointerDelta.x, pointerDelta.y);
                }
            }
        }
...
//                        if (axis == 1)
//                        {
//                            Debug.Log("LateUpdate Elastic m_Velocity :" + m_Velocity + "offset :" + offset);
//                        }
                    }
...
//                        if (axis == 1)
//                        {
//                            Debug.Log("LateUpdate m_Inertia m_Velocity  2222222222222222:" + m_Velocity);
//                            Debug.Log("LateUpdate m_Inertia position  2222222222222222: " + position);
//                        }
                    }
...
//                    Debug.Log("verticalNormalizedPosition totalSize：" + totalSize);
//                    Debug.Log(" verticalNormalizedPosition offset：" + offset);
//                    Debug.Log("verticalNormalizedPosition result：" + (offset - m_ViewBounds.max.y) / (totalSize - m_ViewBounds.size.y));
        ...
        public void SetNormalizedPosition(float value, int axis)
        {
            ...
            //==========LoopScrollRect==========
            Profiling.Profiler.BeginSample("LoopScrollRect SetNormalizedPosition");

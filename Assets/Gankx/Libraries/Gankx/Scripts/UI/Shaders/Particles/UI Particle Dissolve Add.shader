Shader "UI/Particles/Dissolve Additive" {
Properties {
	...
	SubShader {
		...
		Pass {
			...
			{				
				...
				float4 noiseCol = tex2D(_Noise, IN.texcoord);

				float alpha = texCol.a * ((IN.color.a * 1.5 - 0.5) + noiseCol.r);

				clip(alpha - 0.5);

				fixed4 col = IN.color * _TintColor * texCol;
				col.a = alpha;

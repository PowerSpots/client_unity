...
namespace SevenZip
{
    ...
    internal abstract class LzmaBench
	{
		...
		static void PrintValue(UInt64 v, CLogger logger)
		{
			string s = v.ToString();
...
//			System.Console.Write(s);
		...
		static void PrintRating(UInt64 rating, CLogger logger)
		{
            PrintValue(rating / 1000000, logger);
		    logger.Write(" MIPS");
//            System.Console.Write(" MIPS");
		...
		static void PrintResults(
			...
			UInt64 elapsedTime,
			UInt64 size,
			bool decompressMode, UInt64 secondSize, CLogger logger)
		{
			UInt64 speed = MyMultDiv64(size, elapsedTime);
			PrintValue(speed / 1024, logger);
		    logger.Write(" KB/s  ");
//			System.Console.Write(" KB/s  ");
		...
		static public int LzmaBenchmark(Int32 numIterations, UInt32 dictionarySize)
		{
            ...
            logger.Open("./LzmaBench_"+numIterations + "_" + dictionarySize +".log");

			if (dictionarySize < (1 << 18))
			{
                logger.WriteLine("\n\rError: dictionary size for benchmark must be >= 19 (512 KB)");
			    logger.Close();
//				System.Console.WriteLine("\nError: dictionary size for benchmark must be >= 19 (512 KB)");logger.
                return 1;
			}
            logger.Write("\n\r       Compressing                Decompressing\n\r\n\r");
//			System.Console.Write("\n       Compressing                Decompressing\n\n");
			...
			for (Int32 i = 0; i < numIterations; i++)
			{
				...
				PrintResults(dictionarySize, encodeTime, benchSize, false, 0, logger);
//				System.Console.Write("     ");
                logger.Write("     ");
                PrintResults(dictionarySize, decodeTime, kBufferSize, true, (ulong)compressedSize, logger);
//				System.Console.WriteLine();
                logger.WriteLine("");
...
//			System.Console.WriteLine("---------------------------------------------------");
            logger.WriteLine("---------------------------------------------------");
            PrintResults(dictionarySize, totalEncodeTime, totalBenchSize, false, 0, logger);
//			System.Console.Write("     ");
            logger.Write("     ");
            PrintResults(dictionarySize, totalDecodeTime,
					kBufferSize * (UInt64)numIterations, true, totalCompressedSize, logger);
//			System.Console.WriteLine("    Average");
            logger.WriteLine("    Average");
            logger.Close();
            return 0;
		}
	}

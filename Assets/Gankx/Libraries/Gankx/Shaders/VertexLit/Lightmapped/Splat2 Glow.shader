...
Shader "Celf/VertexLit/Lightmapped/Splat2 Glow" {
Properties {
	_Color ("Main Color", Color) = (.5, .5, .5, 1)
    _Splat0 ("Layer1 (RGB)", 2D) = "white" {}
	_Splat1 ("Layer2 (RGB)", 2D) = "white" {}
	_Control ("Control (RG)", 2D) = "white" {}
...
CGINCLUDE
		...
		v2f vert (appdata_full v)
		{
			...
			o.uv[4] = TRANSFORM_TEX (v.texcoord, _Glow);
			return o;
		}
		...
		fixed4 frag (v2f i) : COLOR
		{
			...
			fixed4 glow = tex2D(_Glow,   i.uv[4]);
...
Fallback "Celf/VertexLit/Splat2 Glow"
} 
...
Shader "Celf/Unlit/Transparent/Volumetric Fog"
{
	Properties
	{
		...
		//[HideInInspector]
		_HLDepthTexture ("Depth Tex (Auto)", 2D) = "black" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform float _Offset;
				...
				uniform sampler2D _HLDepthTexture;
			#endif
			...
			fixed4 frag(v2f i) : COLOR0
			{
				...
				#elif defined(VOLUMETRICFOGHL)
					float4 dec = tex2D(_HLDepthTexture, i.pos2.xy/i.pos2.w);
					float depthEye = DecodeFloatRG(dec.rg) * 200.0f;
					float d = depthEye - i.pos2.z - _Offset;
					vFogFactor = CalcFogFactor(d, _FogDensity);
				#endif
	...
	SubShader
	{
		//to render vfog obj back face to depthtexture
		Tags { "Queue" = "Transparent" "DepthMode"="true"}
		Pass {
			Fog { Mode Off }
			Blend Off
			Lighting Off
			...
			fixed4 frag(v2f i) : COLOR {
				...
				//enc.rgba = EncodeFloatRGBA(i.depth.x * 0.01f);
				enc.rg = EncodeFloatRG(i.depth.x * 0.01f);
				return enc;
			}
			ENDCG
		}
